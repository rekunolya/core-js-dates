...
function getNextFriday(date) {
  ...
  const data = new Date(date).setDate(new Date(date).getDate() + countOfDays);
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const dif = (date2 - date1) / (1000 * 60 * 60 * 24) + 1;
  return dif;
}
...
function formatDate(date) {
  ...
  const res = `${month}/${day}/${year}, ${time} ${partOfDay}`;
  return res;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  for (let i = 1; i <= date; i += 1) {
    const dayNum = new Date(year, month - 1, i).getDay();
...
function getNextFridayThe13th(date) {
  ...
  let friday13 = date.getDate();
  for (let month = givenMonth; month < 12; month += 1) {
    const day = new Date(givenYear, month, 13);
    if (day.getDay() === 5) {
      friday13 = day;
      break;
    }
 ...
 */
function formatDateToSchedule(date) {
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (date <= endDate) {
    for (let i = 0; i < countWorkDays; i += 1) {
      ...
      schedule.push(formatDateToSchedule(date));
